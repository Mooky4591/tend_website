...
- The local `paused` state is seeded from `remindersPaused` and must be resynced via `useEffect` keyed on `[remindersPaused, userId]` so a fresh server-rendered value (e.g. after `router.refresh()` or a soft-nav to a different homeowner that reuses this instance) overrides the local optimistic value.
