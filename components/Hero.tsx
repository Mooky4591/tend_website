const smsMessages = [
  {
    ...
    text: "Hi Maria! 👋 I'm Tendr, your AI home assistant from Armadillo Home Warranty. I'll help keep your home at 342 Oak Bluff Dr in great shape. What type of roof do you have, and roughly how old is it?",
    time: '9:01 AM',
  },
  {
    role: 'user' as const,
  ...
  {
    role: 'assistant' as const,
    text: 'Got it — tile roof, ~2014. How many HVAC units do you have?',
    time: '9:03 AM',
  },
  {
    role: 'user' as const,
    text: 'Just one, a Carrier',
    time: '9:05 AM',
  },
  {
    role: 'assistant' as const,
...
function PhoneMockup() {
  return (
    ...
    >
      {/* Phone frame */}
      <div className="relative bg-deep-slate rounded-[2.8rem] p-2.5 shadow-2xl shadow-black/60 border border-deep-slate/50">
        {/* Top notch */}
        ...
        <div className="bg-white rounded-[2.2rem] overflow-hidden pt-8">
          {/* Status bar */}
          <div className="bg-sand px-5 py-2 flex items-center justify-between">
            <span className="text-[10px] font-semibold text-deep-slate">Armadillo Warranty</span>
            <div className="flex items-center gap-1">
              <svg className="w-2.5 h-2.5 text-deep-slate/40" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <svg className="w-2.5 h-2.5 text-deep-slate/40" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
              ...
              >
                ...
                {msg.time === '2 days later' && (
                  <div className="w-full text-center my-1">
                    <span className="text-[9px] text-deep-slate/40 bg-sand px-2 py-0.5 rounded-full">
                      2 days later
                    </span>
                  </div>
                )}
                <div
                  className={`max-w-[85%] px-3 py-2 rounded-2xl text-[11px] leading-relaxed ${
                    msg.role === 'user'
                      ...
                      : 'bg-sand text-deep-slate rounded-tl-sm'
                  }`}
                >
                  {msg.text}
                </div>
                {msg.time !== '2 days later' && (
                  <span className="text-[9px] text-deep-slate/40 px-1">{msg.time}</span>
                )}
              </div>
            ))}
          </div>
          ...
          {/* Input bar */}
          <div className="bg-white border-t border-deep-slate/10 px-3 py-2 flex items-center gap-2">
            <div className="flex-1 bg-sand rounded-full px-3 py-1.5 text-[10px] text-deep-slate/40">
              Text message
            </div>
          ...
          <div className="bg-white pb-2 flex items-center justify-center">
            <div className="w-24 h-1 bg-deep-slate/20 rounded-full" />
          </div>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="text-center lg:text-left">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
              <a
                href="#contact"
                className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-brand-600 text-white font-semibold hover:bg-brand-500 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-overlay"
              >
                Book a Demo
              </a>
              <a
                href="#how-it-works"
                className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg border border-white/30 text-white/80 font-semibold hover:border-white/50 hover:text-white transition-colors"
              >
                See How It Works
