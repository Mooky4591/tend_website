...
export default function Stats() {
  return (
    <section className="bg-white py-16 border-b border-border/10" aria-label="Platform highlights">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <dl className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="text-center px-4 py-6 rounded-xl border border-border/10 bg-muted/50"
            >
              ...
              <dd>
                ...
                <span className="mt-1 block text-sm font-semibold text-foreground" aria-hidden="true">{stat.label}</span>
                <span className="mt-1 block text-xs text-muted-foreground/60 leading-relaxed">{stat.detail}</span>
              </dd>
            </div>
          ))}
        </dl>
