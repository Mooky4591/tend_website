...
- Classify a null field as a gap only when no user message contains a phrase indicating "I don't know" or "I don't have one".
- Skip `washer_dryer_age_years` when `home_details.has_washer_dryer` is `false`.
...
- Do not mark a field as a gap when `has_washer_dryer === false` (for `washer_dryer_age_years`).
